...
const ALang = Language.getString('asena');
const capt = "FB VIDEO DOWNLOADING"
const DOWNMSG = "```DOWNLOADING VIDEO```"
...
// අමාරුවෙන් යකො හැදුවෙ. උස්සන්න එපා තිරිසනෝ 😒
    ...
    if (!link) return await message.sendMessage("හරි ලිංක් එකක් ලබා දෙන්න මෙය වැරදියි")
				...
				await axios 
					...
					.then(async(response) => {
						...
						await message.sendMessage(Buffer.from(alexafb.data), MessageType.video, {caption: capt,})
							.catch(
								async(err) => await message.sendMessage("මෙය වැරදි ලිංක් එකකි"),
							)
					})
