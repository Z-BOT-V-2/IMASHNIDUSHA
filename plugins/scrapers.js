...
if (config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'video ?(.*)', fromMe: false, desc: Lang.VIDEO_DESC}, (async (message, match) => { 
        ...
        yt.on('end', async () => {
            reply = await message.client.sendMessage(message.jid,Lang.UPLOADING_VIDEO,MessageType.text);
            await message.client.sendMessage(message.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video, { quoted: message.data, caption: "සෑ වෑව් ලස්සන වීඩියෝ කෑල්ල🙊\nᴘᴏᴡᴇʀᴅ ʙʏ ᴢʙᴏᴛ" });
        });
    }));
    ...
    Asena.addCommand({pattern: 'song ?(.*)', fromMe: false, desc: Lang.ISONG_DESC}, (async (message, match) => { 
        ...
        ffmpeg(stream)
            ...
            .on('end', async () => {
                ...
                await message.client.sendMessage(message.jid,Buffer.from(writer.arrayBuffer), MessageType.document, {filename: 'Documant' + '.mp3', mimetype: 'audio/mpeg', quoted: message.data});
            });
    }));
    ...
    Asena.addCommand({pattern: 'zimg ?(.*)', fromMe: false, desc: Lang.IMG_DESC}, (async (message, match) => { 
